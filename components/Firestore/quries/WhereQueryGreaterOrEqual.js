...
const WhereQueryGreaterOrEqual = () => {
  const [users, setUsers] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Set up a real-time listener with a filter for users aged 30 or greater
    const unsubscribe = firestore()
      .collection('users')
      .where('age', '>=', 30) // Filter for users aged 30 or greater
...
export default WhereQueryGreaterOrEqual;
